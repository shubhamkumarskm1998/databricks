...
# MAGIC %fs ls 'abfss://ginzobox@deacourseextdlshubh.dfs.core.windows.net/landing/external_data/payments'
...
df = ( spark.read.format('csv').option('delimiter', ',').schema(py_payments_schema).load('abfss://ginzobox@deacourseextdlshubh.dfs.core.windows.net/landing/external_data/payments'))
display(df)
...
df.writeTo('ginzobox.bronze.py_payments').createOrReplace()

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM ginzobox.bronze.py_payments;

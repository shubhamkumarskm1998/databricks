...
df_addresses = spark.table('ginzobox.bronze.py_addresses')
...
df_pivoted_address = df_addresses.groupBy("customer_id").pivot("address_type",['billing','shipping']).agg(f.max("address_line_1").alias("address_line_1"),
...
df_pivoted_address.writeTo('ginzobox.silver.py_addresses').createOrReplace()

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM ginzobox.silver.py_addresses;

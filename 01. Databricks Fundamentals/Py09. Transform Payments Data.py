...
df = spark.table('ginzobox.bronze.py_payments')
display(df)
...
df_extracted_payments = df.select('payment_id','order_id',
...
df_mapped_payments.writeTo('ginzobox.silver.py_payments').createOrReplace()

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM ginzobox.silver.payments;

...
df = spark.sql("select * from ginzobox.bronze.py_customers");
display(df)
...
df = spark.table('ginzobox.bronze.py_customers')
...
df_filtered = df.dropna(subset=['customer_id'])
display (df_filtered)
...
final_df = df_distinct.join(df_max_ts,(df_distinct.customer_id==df_max_ts.customer_id) & 
